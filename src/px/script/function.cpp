...
namespace px::script {
  ...
  priv::FunctionHandle::FunctionHandle(const FunctionHandle &other)
    : FunctionHandle(other.m_ctx, other.m_func, other.m_self) {}

  priv::FunctionHandle &priv::FunctionHandle::operator=(const FunctionHandle &other) {
    if (&other != this) {
      m_self = other.m_self;
      m_func = other.m_func;
      m_ctx  = other.m_ctx;
  ...
  void priv::FunctionHandle::prepare() {
    ...
    CVLOG(9, "AngelScript") << "Preparing context " << m_ctx << " with function " << m_func;

    if (m_self) {
      CVLOG(9, "AngelScript") << "\tSelf: " << m_self;
  ...
  void priv::FunctionHandle::setArg(int arg, std::int8_t value) {
    check();
    CVLOG(9, "AngelScript") << "\tArg[" << arg << "] int8 " << value << " (" << static_cast<std::uint8_t>(value) << ")";
    m_ctx->SetArgByte(arg, value);
  }

  void priv::FunctionHandle::setArg(int arg, std::int16_t value) {
    check();
    CVLOG(9, "AngelScript") << "\tArg[" << arg << "] int16 " << value << " (" << static_cast<std::uint16_t>(value) <<
 ")";
  ...
  void priv::FunctionHandle::setArg(int arg, std::int32_t value) {
    check();
    CVLOG(9, "AngelScript") << "\tArg[" << arg << "] int32 " << value << " (" << static_cast<std::uint32_t>(value) <<
 ")";
  ...
  void priv::FunctionHandle::setArg(int arg, std::int64_t value) {
    check();
    CVLOG(9, "AngelScript") << "\tArg[" << arg << "] int64 " << value << " (" << static_cast<std::uint64_t>(value) <<
 ")";
